...
const ListButton = ({
  ...
  const dynamicMode =
    algo === "Dynamic-Path-finding" && label === "Dynamic-Path-finding";
  const isAStar = ["Manhattan-Distance", "Diagonal-Distance"].includes(label);
  const isActive =
    algo === label || algo === "A*-Algorithm-" + label || dynamicMode;

  const styles = {
    button: {
      display: "flex",
      "&:hover": {
        bgcolor: isAStar && !isActive ? "white" : "white",
  ...
  return (
    <Link to={path}>
      <ListItemButton onClick={() => handleClick(label)} sx={styles.button}>
        {label === "Dynamic-Path-finding" && !isActive && (
          <Box sx={styles.specialTag} />
        )}
