...
const Node = ({
  ...
  function handleClick() {
    if (!midwayActive || isStart || isWall || (!mouseChaseActive && isFinish))
      return;
  ...
  function handleChaseMouseEnter() {
    ...
    dispatch(
      nodeChanged({ row: fRow, col: fCol, att: "isFinish", val: false })
    );
    ...
    const { row: tRow, col: tCol } =
      window.targetNodes[window.targetNodes.length - 1];
  ...
  function handlePointerUp(e) {
    ...
    if (isDragging)
      dispatch(uiChanged({ prop: "board", att: "isDragging", val: false }));
  ...
  return (
    <td
      ...
      onMouseEnter={
        mouseChaseActive ? handleChaseMouseEnter : handleWallMouseEnter
      }
