...
const Main = () => {
  ...
  useEffect(() => {
    if (category === "Sorting" && prevCategory !== "Sorting") {
      batch(() => {
        rotateX !== 0 &&
          dispatch(uiChanged({ prop: "threeD", att: "rotateX", val: 0 }));
        scale !== 1 &&
          dispatch(uiChanged({ prop: "threeD", att: "scale", val: 1 }));
      });
    } else {
      batch(() => {
        rotateX !== 30 &&
        ...
        scale !== 0.9 &&
          dispatch(uiChanged({ prop: "threeD", att: "scale", val: 0.9 }));
      });
    }
    ...
    <>
      <Box
        sx={{
          ...
          height: `calc(100% - ${
            topBar.height + visualPanel.panelHeight - 5
          }px)`,
          transition: "all 0.3s",
          position: "relative",
